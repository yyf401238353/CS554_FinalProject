...
User.findOne({username:"test"}).then(function(user){
  if(!user){
    ...
    let userSeed=new User();
userSeed.username="test";
userSeed.email="test@gmail.com";
userSeed.setPassword("11111111");
userSeed.save(function (err, user){
  if(!err){
    let articleSeed=new Article({
      title:"test",
      decription:"Article for test",
      body:"nothing special",
      tagList:["test"],
      imageURL:"httpL//localhost:3000/null"
    });
    articleSeed.author = user
    articleSeed.save(function(err,article){
      if(!err){
